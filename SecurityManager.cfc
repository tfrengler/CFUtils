<cfcomponent modifier="final" hint="Utility component, used for common security related tasks. Note that a secretKey is generated upon instantiation which is used for the authKey as well as encryption/decryption during the instance's lifetime" >
    ...
    <!--- PRIVATE --->
    <cffunction name="getInterleavedMask" returntype="string" access="private" >
        <cfargument name="mask" type="string" required="true" hint="A string of exactly 24 characters" />

        <cfif len(arguments.mask) IS NOT 24 >
            <cfthrow message="Error generating interleaved mask" detail="Argument 'mask' must be 24 characters" />
        </cfif>

        <cfset var interleavedMask = [] />
        <cfset var interleave = generateSecretKey("AES", 128) />

        <cfloop from="1" to="#len(arguments.mask)#" index="currentCharIndex" >
            <cfset arrayAppend(interleavedMask, interleave[currentCharIndex]) />
            <cfset arrayAppend(interleavedMask, arguments.mask[currentCharIndex]) />
        </cfloop>

        <cfreturn arrayToList(interleavedMask, "") />
    </cffunction>
